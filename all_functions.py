...
#Function apply t-test between patients and healthy persons values in each feature
...
#Function apply t-test between stable and progresion values in each feature
...
#This function is for the first classification problem
#It is to plot two features on 2D coordinates
def plot_var_2d_rev01(v1, v2, input_data, Met):
    
### Arguments for this function ###
#v1: is the first feature
#v2: is the second feature
#input_data: is a datafarme where the features belongs to
#Met: is the method ('n'=normal, 'l'=apply log function, 'p'=apply power function)
...
#This function is for the first classification problem
#It creates a boxplot for patients values and healthy people values in each feature in dataframe X
...
#Function to create a plot for all tests for each patient
...
# Create plot for tests with first progression as reference point
...
#input_df: input data frame
    ...
    return months #This is the database for both plots in this function
...
### Arguments for this function ###
#months: data frame created from previuos function "plot_1st_prog_ref" 
...
# Create plot for tests with first test (B1 test) as reference point
...
### Arguments for this function ###
#months: data frame created from previuos function "plot_B1_test_ref"  
...
# This function is for the first classification probelm to implement leave one out technique
...
# clf:    classifier
    ...
    ax.set_ylabel('Probability', fontsize=16);
    ax.legend(['Blue is Healthy & Red is Patient'], loc='upper left', fontsize=16)
    
    return


#Function for Features selection for Classification Models
...
# LF    : Just for Lasso, if someone would like to get feature coefficients for each feature selection loop using Lasso
    ...
    if LF == 'Yes' and Met == 'Lasso':
        #print('\n', Lasso_features_df)
        Lasso_features_df.to_csv('./results/All_features_selected_by_Lasso') # Exported with indices to seperate  
                                                                             #features in each outer cv loop
    
    return features_df, top_param



# Function for hyperparameters optimization for classifiers
...
# Function for Evaluation of Classifiers by 10-fold cross Validation
def test_classifiers(X_data, y_data, hyp_param, ROC_AUC = 'None'):
    
### Arguments for this function ###
# X_data: Input variables
# y_data: Target variable
# hyp_param: Hyperparameters for all classifiers
# ROC_AUC: If someone would like to plot ROC curve, this parameter should be 'Yes'
    ...
    LR = LogisticRegression(random_state=0)
    pipe[0] = Pipeline([('std', StandardScaler()), ('LR', LR)])
    pipe[0].set_params(**hyp_param[0])

    #SVC Classifier: 
    SvC = SVC(random_state=0)
    pipe[1] = Pipeline([('std', StandardScaler()), ('SvC', SvC)])
    pipe[1].set_params(**hyp_param[1])

    #KNN Classifier:
    KNN = KNeighborsClassifier()
    pipe[2] = Pipeline([('std', StandardScaler()), ('KNN', KNN)])
    pipe[2].set_params(**hyp_param[2])

    #Random Forest Classifier:
    RF = RandomForestClassifier(random_state=0)
    pipe[3] = Pipeline([('std', StandardScaler()), ('RF', RF)])
    pipe[3].set_params(**hyp_param[3])

    #Naive Bayes Classifier:
    NB = GaussianNB()
    pipe[4] = Pipeline([('std', StandardScaler()), ('NB', NB)])
    pipe[4].set_params(**hyp_param[4])
...
# Function for Feature Selection for Regression Models
...
# X: Input variables
# y: Target variable
# Met: is the method ('LassoCV' or 'RidgeCV' or 'ElasticNetCV')
...
# Function for Evaluation of Regressors by 10-fold cross Validation
def test_regressors(X_data, y_data, hyp_param):

### Arguments for this function ###
# X_data: Input variables
# y_data: Target variable
# hyp_param: Hyperparameters for Random Forest regressor
    ...
    pipe[clf_list[0]].set_params(**hyp_param)
